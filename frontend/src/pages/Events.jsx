...
import { Calendar, Award, MapPin, Clock, Plus, History, CheckCircle2, Trophy, Upload, Trash2, Globe, ShieldCheck, Zap, Sparkles, Binary, AlertCircle, Loader2, ExternalLink } from 'lucide-react';
...
const Events = () => {
    ...
    const handleFileChange = async (e) => {
        ...
        uploadData.append('artifact', file);

        setUploading(true);
        try {
            const { data } = await axios.post('/api/events/upload-artifact', uploadData);
            setFormData({ ...formData, certificateUrl: data.artifactUrl });
            setMessage({ type: 'success', text: 'Artifact Uploaded ✓' });
        } catch (err) {
            setMessage({ type: 'error', text: err.response?.data?.message || 'Failed to upload artifact.' });
        } finally {
            setUploading(false);
        }
    };
                                    ...
                                    {formData.certificateUrl ? (
                                        <div className="flex items-center space-x-4">
                                            <a
                                                ...
                                                className="text-[#92400E] font-black py-3 px-6 rounded-[24px] hover:bg-slate-50 border border-transparent hover:border-slate-100 transition-all flex items-center space-x-2 text-[10px]"
                                            >
                                            ...
                                            <button
                                                ...
                                                disabled={uploading}
                                                className="bg-white border border-slate-100 text-slate-900 font-black py-3 px-6 rounded-[24px] hover:bg-slate-50 transition-all flex items-center space-x-2 shadow-sm text-[10px]"
                                            >
                                                {uploading ? <Loader2 size={16} className="animate-spin text-[#92400E]" /> : <Upload size={16} className="text-[#92400E]" />}
                                        ...
                                        <button
                                            ...
                                            onClick={triggerUpload}
                                            disabled={uploading}
                                            className="bg-white border border-slate-100 text-slate-900 font-black py-5 px-10 rounded-[24px] hover:bg-slate-50 transition-all flex items-center space-x-4 shadow-xl hover:-translate-y-1 text-[10px] disabled:opacity-50"
                                        >
                                            {uploading ? (
                                                <Loader2 size={20} className="animate-spin text-[#92400E]" />
                                            ) : (
                                    ...
                                    <div className="flex items-center justify-between pt-8 border-t border-slate-50">
                                        ...
                                        {event.certificateUrl ? (
                                            <a
                                                ...
                                                target="_blank"
                                                rel="noopener noreferrer"
                                                className="text-[#92400E] font-black text-[10px] hover:scale-105 active:scale-95 transition-all flex items-center space-x-3 bg-amber-50 px-5 py-2.5 rounded-2xl border border-amber-100/50"
                                            >
                                                <Award size={16} />
                                                <span>Verify Artifact</span>
                                            </a>
                                        ) : (
                                            <button className="text-slate-400 font-black text-[10px] cursor-not-allowed flex items-center space-x-3 bg-slate-50 px-5 py-2.5 rounded-2xl border border-slate-100/50">
                                                <Award size={16} />
