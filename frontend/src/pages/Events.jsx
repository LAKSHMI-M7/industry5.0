...
const handleFileUpload = async (e) => {
    ...
    // Validation for size (5MB) and type
    ...
    formDataFile.append('artifact', file);
    setUploadingArtifact(true);

    try {
        const { data } = await axios.post('/api/events/upload-artifact', formDataFile, {
            headers: { 'Content-Type': 'multipart/form-data' }
        });
        setFormData(prev => ({ ...prev, certificateUrl: data.url }));
        setMessage({ type: 'success', text: 'Artifact successfully uploaded and linked.' });
    } catch (err) {
        console.error('Artifact upload error:', err);
        setMessage({ type: 'error', text: 'Failed to upload artifact. Please try again.' });
    } finally {
                                ...
                                <div className="flex items-center space-x-6">
                                    ...
                                    <div>
                                        <h4 className="font-black text-slate-900 text-sm">
                                            {formData.certificateUrl ? 'Artifact Uploaded ✓' : 'Digital Credential'}
                                        </h4>
                                        <p className="text-[10px] text-slate-500 font-black mt-1">
                                            {formData.certificateUrl ? 'File securely linked to entry' : 'Institutional verification required'}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4">
                                    {formData.certificateUrl && (
                                        <a
                                            ...
                                            className="bg-slate-100 border border-slate-200 text-slate-700 font-black py-4 px-8 rounded-[24px] hover:bg-slate-200 transition-all flex items-center space-x-3 shadow-sm hover:-translate-y-1 text-[10px]"
                                        >
                                            <span>View File</span>
                                        </a>
                                    )}
                                    <button
                                        ...
                                        className="bg-white border border-slate-100 text-slate-900 font-black py-4 px-8 rounded-[24px] hover:bg-slate-50 transition-all flex items-center space-x-3 shadow-xl hover:-translate-y-1 text-[10px] disabled:opacity-50"
                                    >
                                        {uploadingArtifact ? (
                                        ...
                                        <span>{formData.certificateUrl ? 'Replace File' : 'Link Artifact (PDF/IMG)'}</span>
                                    </button>
                                </div>
                            </div>
                            ...
                            >
                                ...
                                <div className="flex items-center justify-between pt-8 border-t border-slate-50">
                                    ...
                                    {event.certificateUrl ? (
                                        <a href={event.certificateUrl} target="_blank" rel="noopener noreferrer" className="text-[#92400E] font-black text-[10px] hover:scale-105 active:scale-95 transition-all flex items-center space-x-3 bg-amber-50 px-5 py-2.5 rounded-2xl border border-amber-100/50">
                                            <Award size={16} />
                                            <span>Verify Artifact</span>
                                        </a>
                                    ) : (
                                        <button disabled className="text-slate-400 font-black text-[10px] flex items-center space-x-3 bg-slate-50 px-5 py-2.5 rounded-2xl border border-slate-100 opacity-60 cursor-not-allowed">
                                            <Award size={16} />
